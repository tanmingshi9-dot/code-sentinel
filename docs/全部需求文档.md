...
* **[亮点] 增强安全审查 (Enhanced Security Review)**：
    * **AI 主动检测**：通过优化 Prompt，让 LLM 重点关注安全问题。
    * **重点检测项**：
        * 硬编码密钥、Token、API Key（标记为🔴严重问题）
        * SQL 注入、XSS 等常见漏洞
        * 敏感信息泄露风险
    * **详细建议**：不仅指出问题，还提供具体的环境变量替代方案。
    * **设计理念**：相比 Pre-LLM 扫描，AI 审查能更准确理解上下文，减少误报，且开发者已在日常使用中将代码暴露给 AI（Copilot/Cursor），因此发送给 LLM 审查的风险可控。
* **[亮点] 多语言支持**：
...
### Phase 2: Web管理后台与配置能力 (产品化升级)
* **目标**：让产品真正可用，支持多项目接入和可视化管理。
* **核心功能**：
    * **Web 管理后台**：多仓库配置、审查历史查询、统计看板
    * **优化 Prompt 模板**：结构化输出、明确严重程度、提供修复建议
    * **误报反馈机制**：`/false` 命令标记误报、数据记录与分析
* **技术**：React + TailwindCSS 前端，优化现有 Go 后端，可选引入 PostgreSQL。
