...
1. **Web 管理后台**（高优先级）
   - 仓库管理：列表、新增、配置、删除
   - 每个仓库独立配置（LLM 模型、自定义 Prompt、忽略规则）
   - 审查历史查询（按仓库、时间、状态筛选）
   - 统计看板（审查趋势、问题分布、仓库排行）
...
- 后端：复用现有 Go Gin，扩展 API
- 数据库：SQLite（可选升级 PostgreSQL）

**详细需求**：见 `docs/Phase2需求文档.md`

---

### Phase 3: RAG 知识库（3-4周）
**核心目标**：让 AI 理解代码，支持智能问答

**核心功能**：
- 代码向量化与语义搜索
- 智能问答助手（"某功能在哪里实现"）
- 错题本增强（基于 Phase 2 的误报数据，RAG 检索相似场景）
- 相似代码检测（避免重复造轮子）
